...
public class ApplicationContext {
	//application-context.properties 파일을 읽어서 key 값과 value를 담아줄 Hashtable 생성
	...
	public void prepareAnnotationObject() throws Exception {
		//리플렉션 라이브러리 : 리플렉션APII보다 더 쉽게 클래스나 메소드를 찾을 수 있다.
		//new Reflections("패키지명") : 해당 패키지의 하위를 모두 찾는다.
		//new Reflections("spms") : spms패키지의 하위를 모두 찾는다.
		//new Reflections("spms.controller") : spms.controller패키지의 하위를 모두 찾는다.
		//new Reflections("") : classpath 하위를 모두 찾는다.(컴파일 된 클래스를 모두 검사)
		Reflections reflector = new Reflections("");
		
		//@Component 어노테이션을 가진 type을 추출하도록 설정
		Set<Class<?>> list = reflector.getTypesAnnotatedWith(Component.class);
		String key = null;
		for(Class<?> clazz : list) {
			//@Component로 설정된 어노테이션의 value값 추출
