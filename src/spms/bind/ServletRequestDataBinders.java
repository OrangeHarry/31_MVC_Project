...
public class ServletRequestDataBinders {
		...
		if(isPrimitiveType(dataType)) {
			...
			for(String paramName : paramNames) {
				...
				if(m != null) {
					...
					//첫 번째 매개변수에 클라리언트의 매개변수 값을 대입
					//리플렉션 API invoke() : 해당 메소드 실행
					m.invoke(dataObject, //MemberVO값(이메일이나 no, name등등)맞나?
	...
	private static Method findSetter(Class<?> type, String name) {
		...
		for(Method m : methods) {
			...
			propName = m.getName().substring(3);//3인 이유는 set을 빼고 그 다음부터 이용하려고
			//propName = name; (결국 우리가 쓰는 메소드랑 같아지려고..)
	...
	//1-2 바로 객체로 만들어서 리턴 해줌(라인 14,15)
